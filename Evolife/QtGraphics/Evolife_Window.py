...
import Evolife.QtGraphics.Screen as EScreen			 # Physical screens
...
Screen = None	# to be instantiated in 'Start'
...
class Simulation_Control_Frame(Simulation_Control, Evolife_Graphic.Active_Frame):
	...
	def __init__(self, SimulationStep, Obs):
		...
		self.setGeometry(*Screen.locate(200, 200, 140, 300))		
		self.show()
	...
	def HelpButtonClick(self, event=None):
		...
		if not 'Help' in self.SWindows:
			...
			try:
				...
				self.SWindows['Help'].setGeometry(*Screen.locate(400, 120, 600, 500))		
...
class Simulation_Frame(Simulation_Display_Control_Frame):
	...
	def __init__(self, SimulationStep, Obs, Background=None):
		...
		self.setGeometry(*Screen.locate(50, 50, 700, 420))		

		##################################
		# plot panel					 #
		##################################
		self.plot_area= Evolife_Graphic.AreaView(AreaType=Plot_Area.Plot_Area, image=Background, zoom=Screen.ratio)
	...
	def LegendButtonClick(self, event=None):
		...
		if not 'Legend' in self.SWindows:
			...
			try:
				...
				self.SWindows['Legend'].setGeometry(*Screen.locate(50, 550, 600, 150))		
	...
	def Process_graph_orders(self):
		...
		if PlotData:	
			for (CurveId, Point) in PlotData:
				self.plot_area.Area.plot(CurveId, Point, Width=3*Screen.ratio)
			...
			for B in DefViews:
				...
				if type(B) == str:	self.Buttons[B].animateClick()
					...
					if len(B) <= 3:	# only dimensions provided
						self.SWindowsPreferredGeometry[B[0]] = Screen.resize(*B[1:])
					else:
						self.SWindowsPreferredGeometry[B[0]] = Screen.locate(*B[1:5])
					if len(B) > 4:	# margins
	...
	def GenomeButtonClick(self, event):
		...
		if not 'Genomes' in self.SWindows:
			self.SWindows['Genomes'] = Evolife_Graphic.Genome_window(control=self,
																...
																image=self.Background['Genomes'], zoom=Screen.ratio)
			# moving the window
			self.SWindows['Genomes'].move(*Screen.locate(800, 200))		
	...
	def TrajectoryButtonClick(self, event):
		...
		if 'Trajectories' not in self.SWindows:
			self.SWindows['Trajectories'] = Evolife_Graphic.Trajectory_window(control=self, 
												Wtitle='Trajectories', 
												outputDir=self.Obs.get_info('OutputDir'), 
												image=self.Background['Trajectories'], zoom=Screen.ratio)
			# moving the window
			self.SWindows['Trajectories'].move(*Screen.locate(275, 500))		
	...
	def NetworkButtonClick(self, event):
		...
		if 'Network' not in self.SWindows:
			self.SWindows['Network'] = Evolife_Graphic.Network_window(control=self, 
												outputDir=self.Obs.get_info('OutputDir'), 
												image=self.Background['Network'], zoom=Screen.ratio)
			self.WindowActivation('Network')
			self.SWindows['Network'].move(*Screen.locate(790, 500))
	...
	def FieldButtonClick(self, event):
		...
		if 'Field' not in self.SWindows:
			self.SWindows['Field'] = Evolife_Graphic.Field_window(control=self, 
												Wtitle=self.Name, 
												outputDir=self.Obs.get_info('OutputDir'), 
												image=self.Background['Field'], zoom=Screen.ratio)
			# moving the window
			self.SWindows['Field'].move(*Screen.locate(800, 100))		
...
def Start(SimulationStep, Obs, Capabilities='C', Options={}):
	...
	global Screen
	Screen = EScreen.Screen_(MainApp)	# stores physical dimensions of display
	Screen.switchScreen()
