	...
	def RankEffect(self, Rank):
		...
		if Rank >= 0:
			try:	return self.RankEffects[Rank]
				...
				ET.error('S_Signalling: RankEffect', str('Rank == %d' % Rank))
		else:	return (0,0)
...
class Individual(SSim.Social_Individual):
	...
	def update(self, infancy=True):
		...
		# self.SignalLevel = ET.percent(self.Features['SignalInvestment'] * self.Quality)
		self.SignalLevel = ET.noise_mult(self.Signal(), Gbl.Param('PerceptionNoise'))					
		BR = self.bestRecord()
	...
	def Signal(self, Feature=None):
		...
		Comp = ET.percent(100-BC) * self.Quality + BC
		VisibleCompetence = ET.percent(Comp * self.feature(Feature))	   
...
##                (self.ID, self.SignalLevel, IndivOffer, Partner.ID, Partner.SignalLevel, PartnerOffer)
		...
		if not self.get_friend(IndivOffer, Partner, PartnerOffer):
			...
			ET.error('S_Signalling', "Negotiation not respected")
			return # the deal is not made
	...
	def Cost(self, Feature=None):
		return ET.percent(Gbl.Param('SignallingCost') * self.feature(Feature))
	...
	def assessment(self):
		...
		for (Rank,Friend) in enumerate(self.Friends()):
			...
			Friend.Risk = ET.percent(Friend.Risk * (100 - ET.percent(Gbl.Param('CompetenceImpact')
																	   * AgentSocialEffect)))
